...
# 	print("*****************",start_time,doc)
# 	doc = json.loads(doc)
# 	early_by_time=0	
# 	late_by_time=0
# 	shift = doc.get('shift')
# 	shift_start_time = frappe.db.get_value("Shift Type",{'name':shift},"start_time")
# 	frm_start_time = datetime.strptime(('start_time'), '%H:%M:%S').time()
# 	#frm_start_time = datetime.strptime(doc.get('start_time'), '%Y-%m-%d %H:%M:%S').time()
# 	if frm_start_time < shift_start_time:
# 		early_by_time = datetime.combine(date.today(), shift_start_time) - datetime.combine(date.today(), frm_start_time)
...
# 	print("~~~~~~~~~~~~~~~~~~",type(shift_start_time2),type(frm_start_time))
