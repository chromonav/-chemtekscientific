...
def get_data(filters, period_list, partner_doctype):
	...
	for d in sales_users_data:
		...
		if d.brand_name not in brand:
			brand.append(d.brand_name)

	date_field = "transaction_date" if filters.get("doctype") == "Sales Order" else "posting_date"

	actual_data = get_actual_data(filters, item_groups, brand,sales_users, date_field, sales_field)
...
def prepare_data(filters, sales_users_data, actual_data, date_field, period_list, sales_field):
	...
	for d in sales_users_data:
		key = (d.parent, d.item_group,d.brand_name)
...
def get_actual_data(filters, item_groups,brand, sales_users_or_territory_data, date_field, sales_field):
	...
	return frappe.db.sql(
		...
		WHERE
			...
			#child_doc=filters.get("doctype") + " Brand",
			#brand=",".join(["%s"] * len(brand)),
		),
...
def get_parents_data(filters, partner_doctype):
	...
	return frappe.get_all(
		...
		fields=["parent","item_group","brand_name", target_qty_amt_field, "fiscal_year", "distribution_id"],
	)
