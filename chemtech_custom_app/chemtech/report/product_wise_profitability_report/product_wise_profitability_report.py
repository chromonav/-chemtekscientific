    ...
    for row1 in data1:
        if (row1.get('pi_tax')!=None):
            ...
            for i in pi_gst_list:
                pi_test_list = [str(element) for element in i]  
                #pi_new_integer = int(''.join(pi_test_list))
                pi_Final_gst.append(pi_test_list)
                
    #----------- get tax int value ends-------------- 

    #----------- get tax int value for PR starts-------------- 
    pr_gst_list = []
    pr_test_list=[]
    pr_Final_gst=[]
    for row3 in data3:
        if (row3.get('pr_tax')!=None):
            pr_temp = re.findall(r'\d+', row3.get('pr_tax'))
            pr_res = list(map(int, pr_temp))
            pr_gst_list.append(pr_res)
            for i in pr_gst_list:
                pr_test_list = [str(element) for element in i]  
                #pi_new_integer = int(''.join(pi_test_list))
                pr_Final_gst.append(pr_test_list)
    ...
    for row in data:
        ...
        for row1 in data1:
            if row.get('item_code') == row1.get('item_code') and row.get('company')==row1.get('company'):
                row.update({'purchase_invoice':(row1.get('purchase_invoice'))})
                row.update({'pi_rate':(row1.get('pi_rate'))})
                row.update({'purchase_taxable_value':(row1.get('pi_rate')*(row.get('si_qty')))})
                row.update({'pi_tax':(row1.get('pi_tax'))})
                if row.get('pi_tax')!=None:
                    for i in pi_gst_list:
                        for j in i:
                            row.update({'pi_gst_tax_amount':(int(row.get('purchase_taxable_value')*flt(j/100)))})
                    ...
                    #print("---no pi_tax-----")
                    row.update({'pi_total_value':(flt(row.get('purchase_taxable_value')))})
                    row.update({'gross_profit':(flt(row.get('si_total_value')-row.get('pi_total_value')))})
            ...
            for row3 in data3:
                if row.get('purchase_invoice') == None and row.get('stock_voucher')==None:
                    if (row.get('item_code')==row3.get('item_code')):
                        ...
                        row.update({'stock_purpose':'Purchase Receipt'})
                        row.update({'pi_rate':row3.get('purchase_rec_rate')})
                        row.update({'pi_tax':row3.get('pr_tax')})
                        row.update({'purchase_taxable_value':(flt(row.get('pi_rate')*row.get('si_qty')))})
                        if row3.get('pr_tax')!=None:
                            print("-------in pr_tax != None--------")
                            for i in pr_gst_list:
                                for j in i:
                                    print("--------IN pi_gst_list J==---------")
                                    row.update({'pi_gst_tax_amount':(int(row.get('purchase_taxable_value')*flt(j/100)))})
                            row.update({'pi_total_value':(flt(row.get('purchase_taxable_value')+row.get('pi_gst_tax_amount')))})
                        row.update({'gross_profit':(flt(row.get('si_total_value')-row.get('pi_total_value')))})
...
def get_columns(filters):
    columns = [
        ...
        {
            "label": _("Stock Opening/Transfer"),
