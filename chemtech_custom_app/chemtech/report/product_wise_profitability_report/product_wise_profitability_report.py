...
def get_data(filters):
	...
	JOIN `tabAddress` ad ON si.customer_address = ad.name WHERE posting_date BETWEEN '{0}' and '{1}' and si.company='{2}' """.format(filters.get('from_date'), filters.get('to_date'),filters.get('company')),as_dict=1,debug=1)
	...
	join `tabPurchase Order` po  on pii.purchase_order=po.name join `tabSales Invoice` si  on po.company=si.company where po.company='{1}' and  pii.item_code in {0} and po.transaction_date<si.posting_date """.format(tuple(item_code_list),filters.get('company')),as_dict=1)
	#data2 = frappe.db.sql(""" SELECT DISTINCT pii.item_code,pii.purchase_order,po.name as purchase_order,po.company,po.transaction_date from `tabPurchase Invoice Item` pii join `tabPurchase Order` po  on pii.purchase_order=po.name join `tabSales Invoice` si  on po.company=si.company where po.company='{1}' and  pii.item_code in {0} and po.transaction_date<si.posting_date """.format(tuple(item_code_list),filters.get('company')),as_dict=1)
    ...
    # data = frappe.db.sql(""" SELECT si.posting_date,MONTHNAME(si.posting_date) as posting_date_month,si.company,si.name as sales_invoice_name,si.customer_name,
    #     ad.city as location,sii.item_code as item_code,sii.brand as si_brand,sii.item_name,sii.rate as si_rate,
    ...
    #      sii.item_code ORDER BY si.posting_date""".format(filters.get('from_date'), filters.get('to_date'),filters.get('company')),as_dict=1)
    # item_code_list = [item.item_code for item in data]
    ...
    #  .format(filters.get('company'),tuple(item_code_list),filters.get('from_date'), filters.get('to_date'),filters.get('company')),as_dict=1)
